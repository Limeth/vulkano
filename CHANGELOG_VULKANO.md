...
- Removed `PipelineLayoutAbstract`, use `Arc<PipelineLayout>` instead.
- Added `PipelineLayoutDescAggregation`, which is accessible using the
  `PipelineLayout::desc` function and implements `Eq` and `Hash`.

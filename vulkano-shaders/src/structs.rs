...
fn write_struct(doc: &Spirv, struct_id: u32, members: &[u32]) -> (TokenStream, Option<usize>) {
	...
	for (num, &member) in members.iter().enumerate() {
		...
		rust_members.push(Member { name: Ident::new(&member_name, Span::call_site()), ty });
	}
...
pub fn type_from_id(doc: &Spirv, searched: u32) -> (TokenStream, Option<usize>, usize) {
	for instruction in doc.instructions.iter() {
		match instruction {
			...
			&Instruction::TypeArray { result_id, type_id, length_id } if result_id == searched => {
				debug_assert_eq!(mem::align_of::<[u32; 3]>(), mem::align_of::<u32>());
			...
			&Instruction::TypeStruct { result_id, ref member_types } if result_id == searched => {
				// TODO: take the Offset member decorate into account?
				let name =
					Ident::new(&spirv_search::name_from_id(doc, result_id), Span::call_site());
				let ty = quote! { #name };

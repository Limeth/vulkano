...
use crate::{
	enums::{Decoration, Dim, ImageFormat, StorageClass},
	parse::{Instruction, Spirv},
	spirv_search
};
...
pub fn write_descriptor_sets(doc: &Spirv) -> TokenStream {
	...
	for set_decoration in doc.get_decorations(Decoration::DecorationDescriptorSet) {
		...
		let binding =
			doc.get_decoration_params(variable_id, Decoration::DecorationBinding).unwrap()[0];

		// Find information about the kind of binding for this descriptor.
		let (desc_ty, readonly, array_count) =
			descriptor_infos(doc, pointed_ty, false).expect(&format!(
...
fn pointer_variable_ty(doc: &Spirv, variable: u32) -> u32 {
	let var_ty = doc
		...
		.filter_map(|i| match i {
			&Instruction::Variable { result_type_id, result_id, .. } if result_id == variable => {
				Some(result_type_id)
			}
			_ => None
		})
	...
	doc.instructions
		.iter()
		.filter_map(|i| match i {
			&Instruction::TypePointer { result_id, type_id, .. } if result_id == var_ty => {
				Some(type_id)
			}
			_ => None
		})
...
fn descriptor_infos(
	doc: &Spirv, pointed_ty: u32, force_combined_image_sampled: bool

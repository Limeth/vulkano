...
pub fn parse_spirv(i: &[u32]) -> Result<Spirv, ParseError> {
	...
	Ok(Spirv { version, bound: i[3], instructions })
}
...
fn decode_instruction(opcode: u16, operands: &[u32]) -> Result<Instruction, ParseError> {
	Ok(match opcode {
		...
		48 => Instruction::SpecConstantTrue { result_type_id: operands[0], result_id: operands[1] },
		49 => {
			Instruction::SpecConstantFalse { result_type_id: operands[0], result_id: operands[1] }
		}
...
impl Spirv {
			...
			if let Instruction::Decorate { target_id, ref decoration, ref params } = instruction {
				if *decoration == find_decoration {
					...
					for target_id in target_ids {
						decorations.push(FoundDecoration { target_id, params: params.clone() });
					}
				}
			}
		}
	...
	pub(crate) fn get_decoration_params(
		&self, id: u32, find_decoration: Decoration
	) -> Option<Vec<u32>> {
		for instruction in &self.instructions {
			match instruction {
	...
	pub(crate) fn get_member_decoration_params(
		&self, struct_id: u32, member_literal: u32, find_decoration: Decoration
	) -> Option<Vec<u32>> {
		for instruction in &self.instructions {
			match instruction {
		...
		for instruction in &self.instructions {
			match instruction {
				Instruction::MemberDecorate {
					...
					ref params,
					..
				} if *target_id == struct_id => return Some(params.clone()),
				Instruction::GroupMemberDecorate { decoration_group, ref targets } => {

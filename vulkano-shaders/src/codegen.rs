...
use crate::{enums::Capability, parse::Instruction};

use crate::{descriptor_sets, entry_point, parse, read_file_to_string, spec_consts, structs};

fn include_callback(
	requested_source_path_raw: &str, directive_type: IncludeType, contained_within_path_raw: &str,
	recursion_depth: usize, include_directories: &[String], root_source_has_path: bool
	...
	let file_to_include = match directive_type {
		IncludeType::Relative => {
			...
			let parent_of_current_source =
				Path::new(contained_within_path_raw).parent().unwrap_or_else(|| {
					panic!(
			...
			let resolved_requested_source_path =
				parent_of_current_source.join(requested_source_path);
		...
		IncludeType::Standard => {
			...
			for include_directory in include_directories {
				let include_directory_path =
					Path::new(include_directory).canonicalize().unwrap_or_else(|_| {
						panic!(
							"Invalid standard shader inclusion directory `{}`.",
							include_directory
						)
					});
				let resolved_requested_source_path_rel =
					include_directory_path.join(requested_source_path);
				let resolved_requested_source_path =
					resolved_requested_source_path_rel.canonicalize().map_err(|_| {
						format!(
				...
				if !resolved_requested_source_path.starts_with(include_directory_path) {
					return Err(format!(
						"Cannot use `..` with inclusion from standard directories \
						 (`#include <...>`), try using `#include \"...\"` instead. \
						 Requested path: {}",
						requested_source_path.to_string_lossy()
					))
				}
	...
	let canonical_file_to_include =
		file_to_include.canonicalize().unwrap_or_else(|_| file_to_include);
	let canonical_file_to_include_string = canonical_file_to_include
		.to_str()
		.expect(
		...
		.to_string();
	let content = read_file_to_string(canonical_file_to_include.as_path()).map_err(|_| {
		format!(
	...
	Ok(ResolvedInclude { resolved_name: canonical_file_to_include_string, content })
}

pub fn compile(
	path: Option<String>, code: &str, ty: ShaderKind, include_directories: &[String]
...
pub fn reflect(name: &str, spirv: &[u32], dump: bool) -> Result<TokenStream, Error> {
	...
	let ast = quote! {
		...
		#[allow(unused_imports)]
		use vulkano::image::ImageDimensionsType;
		#[allow(unused_imports)]
...
impl From<IoError> for Error {
	fn from(err: IoError) -> Error { Error::IoError(err) }
}
...
fn capability_name(cap: &Capability) -> Option<&'static str> {
	match *cap {
		...
		Capability::CapabilityStorageImageMultisample => Some("shader_storage_image_multisample"),

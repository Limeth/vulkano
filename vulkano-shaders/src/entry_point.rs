...
use crate::{
	enums::{Decoration, ExecutionMode, ExecutionModel, StorageClass},
					...
					match *execution {
						...
						ExecutionModel::ExecutionModelGeometry => {
							...
							for instruction in doc.instructions.iter() {
								if let &Instruction::ExecutionMode { target_id, ref mode, .. } =
									instruction
								{
									if target_id == id {
...
fn write_interface_structs(
	doc: &Spirv, capitalized_ep_name: &str, interface: &[u32], ignore_first_array_in: bool,
	...
	for interface in interface.iter() {
		for i in doc.instructions.iter() {
			match i {
				&Instruction::Variable { result_type_id, result_id, ref storage_class, .. }
					if &result_id == interface =>
				{
						...
						.get_decoration_params(result_id, Decoration::DecorationLocation)
					{
						Some(l) => l[0],
						None => {
							panic!("Attribute `{}` (id {}) is missing a location", name, result_id)
						}
					};

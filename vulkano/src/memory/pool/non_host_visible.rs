...
impl StdNonHostVisibleMemoryTypePool {
	...
	pub fn new(
		device: Arc<Device>, memory_type: MemoryType
	...
	pub fn alloc(
		me: &Arc<Self>, size: usize, alignment: usize
...
impl StdNonHostVisibleMemoryTypePoolAlloc {
	pub fn memory(&self) -> &DeviceMemory { &self.memory }
...
impl Drop for StdNonHostVisibleMemoryTypePoolAlloc {
	fn drop(&mut self) {
		...
		let entries =
			occupied.iter_mut().find(|e| &*e.0 as *const DeviceMemory == &*self.memory).unwrap();

...
pub use self::{
	host_visible::{StdHostVisibleMemoryTypePool, StdHostVisibleMemoryTypePoolAlloc},
	non_host_visible::{StdNonHostVisibleMemoryTypePool, StdNonHostVisibleMemoryTypePoolAlloc},
	pool::{StdMemoryPool, StdMemoryPoolAlloc}
};
...
pub unsafe trait MemoryPool: DeviceOwned {
	...
	fn alloc_generic(
		&self, ty: MemoryType, size: usize, alignment: usize, layout: AllocLayout,
		...
		F: FnMut(MemoryType) -> AllocFromRequirementsFilter
	{
		...
		{
			let alloc =
				self.alloc_generic(mem_ty, requirements.size, requirements.alignment, layout, map)?;

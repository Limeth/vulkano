...
impl DeviceMemory {
	...
	pub fn alloc(
		device: Arc<Device>, memory_type: MemoryType, size: usize
	...
	pub fn dedicated_alloc(
		device: Arc<Device>, memory_type: MemoryType, size: usize, resource: DedicatedAlloc
	) -> Result<DeviceMemory, DeviceMemoryAllocError> {
		assert!(size >= 1);
		assert_eq!(
		...
		Ok(DeviceMemory { memory, device, size, memory_type_index: memory_type.id() })
	}
	...
	pub fn dedicated_alloc_and_map(
		...
		Ok(MappedDeviceMemory { memory: mem, pointer: ptr, coherent })
	}
...
unsafe impl VulkanObject for DeviceMemory {
	...
	fn internal_object(&self) -> vk::DeviceMemory { self.memory }
}
	...
	fn drop(&mut self) {
		unsafe {
			...
			let mut allocation_count =
				self.device.allocation_count().lock().expect("Poisoned mutex");
			*allocation_count -= 1;
		}
	}
}
...
impl MappedDeviceMemory {
		...
		T: Content
	{
		...
		CpuAccess { pointer, mem: self, coherent: self.coherent, range }
	}
}

impl AsRef<DeviceMemory> for MappedDeviceMemory {
	fn as_ref(&self) -> &DeviceMemory { &self.memory }
}

impl AsMut<DeviceMemory> for MappedDeviceMemory {
	fn as_mut(&mut self) -> &mut DeviceMemory { &mut self.memory }
}

unsafe impl DeviceOwned for MappedDeviceMemory {
	fn device(&self) -> &Arc<Device> { self.memory.device() }
}
...
impl fmt::Debug for MappedDeviceMemory {
	fn fmt(&self, fmt: &mut fmt::Formatter) -> fmt::Result {
		fmt.debug_tuple("MappedDeviceMemory").field(&self.memory).finish()
	}
}
...
mod tests {
	use crate::{
		memory::{DeviceMemory, DeviceMemoryAllocError},
		OomError
	};

...
use crate::{
	...
	instance::{
		limits::Limits,
		loader::{self, FunctionPointers, Loader, LoadingError},
		InstanceExtensions,
...
/// let layers: Vec<_> =
/// 	instance::layers_list()?.filter(|l| l.description().contains("foo")).collect();
///
...
impl Instance {
	...
	pub fn with_loader<'a, L, Ext>(
		loader: FunctionPointers<Box<Loader + Send + Sync>>, app_infos: Option<&ApplicationInfo>,
		extensions: Ext, layers: L
	) -> Result<Arc<Instance>, InstanceCreationError>
	where
		L: IntoIterator<Item = &'a str>,
		Ext: Into<RawInstanceExtensions>
	{
		...
		Instance::new_inner(app_infos, extensions.into(), layers, OwnedOrRef::Owned(loader))
	}

	fn new_inner(
		app_infos: Option<&ApplicationInfo>, extensions: RawInstanceExtensions,
		layers: SmallVec<[CString; 16]>,
		function_pointers: OwnedOrRef<FunctionPointers<Box<Loader + Send + Sync>>>
	) -> Result<Arc<Instance>, InstanceCreationError> {
		...
		let layers_ptr = layers.iter().map(|layer| layer.as_ptr()).collect::<SmallVec<[_; 16]>>();

		let extensions_list =
			extensions.iter().map(|extension| extension.as_ptr()).collect::<SmallVec<[_; 32]>>();
	...
	fn init_physical_devices(
		vk: &vk::InstancePointers, physical_devices: Vec<vk::PhysicalDevice>
	...
	pub(crate) fn pointers(&self) -> &vk::InstancePointers { &self.vk }
	...
	/// ```
	pub fn loaded_extensions(&self) -> &InstanceExtensions { &self.extensions }

	/// Returns the list of layers requested when creating this instance.
	#[doc(hidden)]
	pub fn loaded_layers(&self) -> slice::Iter<CString> { self.layers.iter() }
}
...
unsafe impl VulkanObject for Instance {
	...
	fn internal_object(&self) -> vk::Instance { self.instance }
}
...
impl fmt::Display for InstanceCreationError {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		match self {
			InstanceCreationError::LoadingError(e) => e.fmt(f),
			InstanceCreationError::OomError(e) => e.fmt(f),

			InstanceCreationError::InitializationFailed => write!(f, "Initialization failed"),
			InstanceCreationError::LayerNotPresent => write!(f, "Layer not present"),
			InstanceCreationError::ExtensionNotPresent => write!(f, "Extension not present"),
			InstanceCreationError::IncompatibleDriver => write!(f, "Incompatible driver")
		}
	}
}
impl error::Error for InstanceCreationError {
	fn source(&self) -> Option<&(dyn error::Error + 'static)> {
		match self {
			InstanceCreationError::LoadingError(e) => e.source(),
			InstanceCreationError::OomError(e) => e.source(),
			_ => None
		}
	}
}
...
impl From<OomError> for InstanceCreationError {
	fn from(err: OomError) -> InstanceCreationError { InstanceCreationError::OomError(err) }
}
impl From<LoadingError> for InstanceCreationError {
	fn from(err: LoadingError) -> InstanceCreationError { InstanceCreationError::LoadingError(err) }
}
...
impl<'a> PhysicalDevice<'a> {
	...
	pub fn enumerate(instance: &'a Arc<Instance>) -> PhysicalDevicesIter<'a> {
		PhysicalDevicesIter { instance, current_id: 0 }
	}
	...
	pub fn from_index(instance: &'a Arc<Instance>, index: usize) -> Option<PhysicalDevice<'a>> {
		if instance.physical_devices.len() > index {
			Some(PhysicalDevice { instance, device: index })
		} else {
			None
		}
	}
	...
	/// 	}
	/// ```
	pub fn instance(&self) -> &'a Arc<Instance> { &self.instance }
	...
	pub fn index(&self) -> usize { self.device }
	...
	pub fn supported_features(&self) -> &'a Features { &self.infos().available_features }
	...
	pub fn queue_families(&self) -> QueueFamiliesIter<'a> {
		QueueFamiliesIter { physical_device: *self, current_id: 0 }
	}
	...
	pub fn queue_family_by_id(&self, id: u32) -> Option<QueueFamily<'a>> {
		if (id as usize) < self.infos().queue_families.len() {
			Some(QueueFamily { physical_device: *self, id })
		} else {
			None
		}
	}
	...
	pub fn memory_types(&self) -> MemoryTypesIter<'a> {
		MemoryTypesIter { physical_device: *self, current_id: 0 }
	}
	...
	pub fn memory_type_by_id(&self, id: u32) -> Option<MemoryType<'a>> {
		if id < self.infos().memory.memoryTypeCount {
			Some(MemoryType { physical_device: *self, id })
		} else {
			None
		}
	}
	...
	pub fn memory_heaps(&self) -> MemoryHeapsIter<'a> {
		MemoryHeapsIter { physical_device: *self, current_id: 0 }
	}
	...
	pub fn memory_heap_by_id(&self, id: u32) -> Option<MemoryHeap<'a>> {
		if id < self.infos().memory.memoryHeapCount {
			Some(MemoryHeap { physical_device: *self, id })
		} else {
			None
		}
	}
	...
	pub fn limits(&self) -> Limits<'a> { Limits::from_vk_limits(&self.infos().properties.limits) }
	...
	/// example.
	pub fn driver_version(&self) -> u32 { self.infos().properties.driverVersion }

	/// Returns the PCI ID of the device.
	pub fn pci_device_id(&self) -> u32 { self.infos().properties.deviceID }

	/// Returns the PCI ID of the vendor.
	pub fn pci_vendor_id(&self) -> u32 { self.infos().properties.vendorID }
	...
	fn infos(&self) -> &'a PhysicalDeviceInfos { &self.instance.physical_devices[self.device] }
}
...
unsafe impl<'a> VulkanObject for PhysicalDevice<'a> {
	...
	fn internal_object(&self) -> vk::PhysicalDevice { self.infos().device }
}
...
impl<'a> Iterator for PhysicalDevicesIter<'a> {
	...
	fn next(&mut self) -> Option<PhysicalDevice<'a>> {
		...
		let dev = PhysicalDevice { instance: self.instance, device: self.current_id };
...
impl<'a> QueueFamily<'a> {
	...
	pub fn physical_device(&self) -> PhysicalDevice<'a> { self.physical_device }
	...
	pub fn supports_graphics(&self) -> bool { (self.flags() & vk::QUEUE_GRAPHICS_BIT) != 0 }

	/// Returns true if queues of this family can execute compute operations.
	pub fn supports_compute(&self) -> bool { (self.flags() & vk::QUEUE_COMPUTE_BIT) != 0 }
...
impl<'a> Iterator for QueueFamiliesIter<'a> {
	...
	fn next(&mut self) -> Option<QueueFamily<'a>> {
		...
		let dev = QueueFamily { physical_device: self.physical_device, id: self.current_id };
...
impl<'a> Iterator for MemoryTypesIter<'a> {
	...
	fn next(&mut self) -> Option<MemoryType<'a>> {
		...
		let dev = MemoryType { physical_device: self.physical_device, id: self.current_id };
...
impl<'a> Iterator for MemoryHeapsIter<'a> {
	...
	fn next(&mut self) -> Option<MemoryHeap<'a>> {
		...
		let dev = MemoryHeap { physical_device: self.physical_device, id: self.current_id };
...
mod tests {
	...
	#[test]
	fn create_instance() { let _ = instance!(); }

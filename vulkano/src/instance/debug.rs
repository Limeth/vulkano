...
use crate::{check_errors, instance::Instance, Error, VulkanObject};
...
impl DebugCallback {
	...
	pub fn new<F>(
		instance: &Arc<Instance>, messages: MessageTypes, user_callback: F
		...
		F: Fn(&Message) + 'static + Send + panic::RefUnwindSafe
	{
		...
		extern "system" fn callback(
			ty: vk::DebugReportFlagsEXT, _: vk::DebugReportObjectTypeEXT, _: u64, _: usize, _: i32,
			layer_prefix: *const c_char, description: *const c_char, user_data: *mut c_void
		) -> u32 {
			unsafe {
				...
				let description =
					CStr::from_ptr(description).to_str().expect("debug callback message not utf-8");
		...
		Ok(DebugCallback { instance: instance.clone(), debug_report_callback, user_callback })
	}
	...
	pub fn errors_and_warnings<F>(
		instance: &Arc<Instance>, user_callback: F
	) -> Result<DebugCallback, DebugCallbackCreationError>
	where
...
impl MessageTypes {
	...
	pub fn errors() -> MessageTypes { MessageTypes { error: true, ..MessageTypes::none() } }
...
impl fmt::Display for DebugCallbackCreationError {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		write!(f, "The `EXT_debug_report` extension was not enabled")
	}
}
...
impl From<Error> for DebugCallbackCreationError {
	fn from(err: Error) -> DebugCallbackCreationError { panic!("unexpected error: {:?}", err) }
}

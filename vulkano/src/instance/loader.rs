...
pub unsafe trait Loader {
	...
	fn get_instance_proc_addr(
		&self, instance: vk::Instance, name: *const c_char
	) -> extern "system" fn() -> ();
}
...
impl DynamicLibraryLoader {
		...
		P: AsRef<Path>
	{
		...
		Ok(DynamicLibraryLoader { vk_lib, get_proc_addr })
	}
}
...
impl<L> FunctionPointers<L> {
		...
		L: Loader
	{
		...
		FunctionPointers { loader, entry_points }
	}

	/// Returns the collection of Vulkan entry points from the Vulkan loader.
	pub(crate) fn entry_points(&self) -> &vk::EntryPoints { &self.entry_points }
	...
	() => {{
		extern "C" {
			fn vkGetInstanceProcAddr(
				instance: vk::Instance, pName: *const c_char
...
/// load Vulkan.
pub fn auto_loader() -> Result<&'static FunctionPointers<Box<Loader + Send + Sync>>, LoadingError> {
	#[cfg(target_os = "ios")]
	...
	fn def_loader_impl() -> Result<Box<Loader + Send + Sync>, LoadingError> {
		#[cfg(windows)]
		fn get_path() -> &'static Path { Path::new("vulkan-1.dll") }
		#[cfg(all(unix, not(target_os = "android"), not(target_os = "macos")))]
		fn get_path() -> &'static Path { Path::new("libvulkan.so.1") }
		#[cfg(target_os = "macos")]
		fn get_path() -> &'static Path { Path::new("libvulkan.1.dylib") }
		#[cfg(target_os = "android")]
		fn get_path() -> &'static Path { Path::new("libvulkan.so") }
...
impl fmt::Display for LoadingError {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		match self {
			LoadingError::LibraryLoadFailure(e) => write!(f, "Failed to load the Vulkan shared library: {}", e),
			LoadingError::MissingEntryPoint(e) => write!(f, "one of the entry points required to be supported by the Vulkan implementation is missing: {}", e)
		}
	}
}
...
#[cfg(test)]
mod tests {
	use crate::instance::loader::{DynamicLibraryLoader, LoadingError};

...
use std::{error, fmt, mem, vec::IntoIter as VecIntoIter};
...
impl fmt::Display for IncompatiblePixelsType {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		write!(f, "Supplied pixel type is incopatible with this format")
	}
}
...
pub unsafe trait FormatDesc {
	...
	fn is_float(&self) -> bool { self.format().ty() == FormatTy::Float }
	fn is_uint(&self) -> bool { self.format().ty() == FormatTy::Uint }
	fn is_sint(&self) -> bool { self.format().ty() == FormatTy::Sint }
	fn has_color(&self) -> bool { self.is_float() || self.is_uint() || self.is_sint() } // TODO: Is this enough?
...
unsafe impl FormatDesc for Format {
	...
	fn format(&self) -> Format { *self }

	fn decode_clear_value(&self, value: Self::ClearValue) -> ClearValue {
		match (self.ty(), value) {
			(FormatTy::Float, ClearValue::Float(_)) => value,
			(FormatTy::Compressed, ClearValue::Float(_)) => value,
			(FormatTy::Sint, ClearValue::Int(_)) => value,
			(FormatTy::Uint, ClearValue::Uint(_)) => value,
			(FormatTy::Depth, ClearValue::Depth(_)) => value,
			(FormatTy::Stencil, ClearValue::Stencil(_)) => value,
			(FormatTy::DepthStencil, ClearValue::DepthStencil(_)) => value,
...
impl From<[f32; 1]> for ClearValue {
	fn from(val: [f32; 1]) -> ClearValue { ClearValue::Float([val[0], 0.0, 0.0, 1.0]) }
}
impl From<[f32; 2]> for ClearValue {
	fn from(val: [f32; 2]) -> ClearValue { ClearValue::Float([val[0], val[1], 0.0, 1.0]) }
}
impl From<[f32; 3]> for ClearValue {
	fn from(val: [f32; 3]) -> ClearValue { ClearValue::Float([val[0], val[1], val[2], 1.0]) }
}
impl From<[f32; 4]> for ClearValue {
	fn from(val: [f32; 4]) -> ClearValue { ClearValue::Float(val) }
}
...
impl From<[u32; 4]> for ClearValue {
	fn from(val: [u32; 4]) -> ClearValue { ClearValue::Uint(val) }
}
...
impl From<[i32; 4]> for ClearValue {
	fn from(val: [i32; 4]) -> ClearValue { ClearValue::Int(val) }
}
impl From<f32> for ClearValue {
	fn from(val: f32) -> ClearValue { ClearValue::Depth(val) }
}
impl From<u32> for ClearValue {
	fn from(val: u32) -> ClearValue { ClearValue::Stencil(val) }
}
impl From<(f32, u32)> for ClearValue {
	fn from(val: (f32, u32)) -> ClearValue { ClearValue::DepthStencil(val) }
}

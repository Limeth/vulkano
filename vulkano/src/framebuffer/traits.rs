...
use crate::{
	device::DeviceOwned,
	format::ClearValue,
	framebuffer::{FramebufferSys, RenderPassDesc, RenderPassSys},
	image::ImageViewAccess,
	pipeline::shader::ShaderInterfaceDef
};
...
pub unsafe trait FramebufferAbstract: RenderPassAbstract {
	...
	fn layers(&self) -> u32 { self.dimensions()[2] }
}
...
{
	fn inner(&self) -> FramebufferSys { FramebufferAbstract::inner(&**self) }
...
/// ```
/// use std::sync::Arc;
/// use vulkano::framebuffer::{EmptySinglePassRenderPassDesc, RenderPass, RenderPassAbstract};
///
...
{
	fn inner(&self) -> RenderPassSys { (**self).inner() }
}
...
pub unsafe trait RenderPassDescClearValues<C> {
	...
	// TODO: meh for boxing
	fn convert_clear_values(&self, c: C) -> Box<Iterator<Item = ClearValue>>;
}
	...
	fn is_compatible_with(&self, subpass: u32, other: &B) -> bool {
		...
		for element in other.elements() {
			for location in element.location.clone() {
				...
				let attachment_desc =
					(&self).attachment_descs().skip(attachment_id).next().unwrap();
	...
	L: RenderPassDesc
{
	...
	pub fn from(render_pass: L, id: u32) -> Option<Subpass<L>> {
		if (id as usize) < render_pass.num_subpasses() {
			Some(Subpass { render_pass, subpass_id: id })
		} else {
			None
		}
	}
	...
	pub fn has_depth(&self) -> bool { self.render_pass.has_depth(self.subpass_id).unwrap() }
	...
	pub fn has_stencil(&self) -> bool { self.render_pass.has_stencil(self.subpass_id).unwrap() }
	...
	pub fn has_color_or_depth_stencil_attachment(&self) -> bool {
		self.num_color_attachments() >= 1
			|| self.render_pass.has_depth_stencil_attachment(self.subpass_id).unwrap()
				!= (false, false)
	}
	...
	pub fn num_samples(&self) -> Option<u32> { self.render_pass.num_samples(self.subpass_id) }
}

impl<L> Subpass<L> {
	/// Returns the render pass of this subpass.
	pub fn render_pass(&self) -> &L { &self.render_pass }

	/// Returns the index of this subpass within the renderpass.
	pub fn index(&self) -> u32 { self.subpass_id }
}

impl<L> Into<(L, u32)> for Subpass<L> {
	fn into(self) -> (L, u32) { (self.render_pass, self.subpass_id) }
}

...
use crate::{
	device::Device,
	format::{ClearValue, Format, FormatTy},
	framebuffer::{
		RenderPass,
	...
	T::Target: RenderPassDesc
{
	...
	fn num_subpasses(&self) -> usize { (**self).num_subpasses() }

	fn subpass_desc(&self, num: usize) -> Option<PassDescription> { (**self).subpass_desc(num) }

	fn num_dependencies(&self) -> usize { (**self).num_dependencies() }

	...
	D: RenderPassDesc
{
	...
	/// mode.
	pub fn new(
		device: Arc<Device>, description: D
		...
		let attachment_references = description
			...
			.flat_map(|pass| {
				...
				debug_assert!(
					pass.resolve_attachments.is_empty()
						|| pass.color_attachments.iter().all(|a| attachments[a.0].samples > 1)
				);
				debug_assert!(
					pass.resolve_attachments.is_empty()
						|| pass
		...
		let passes = unsafe {
			...
			for pass in description.subpass_descs() {
				...
				let preserve_attachments =
					preserve_attachments_references.as_ptr().offset(preserve_ref_index as isize);
...
where
	D: RenderPassDesc
{
	fn num_attachments(&self) -> usize { self.desc.num_attachments() }

	fn attachment_desc(&self, num: usize) -> Option<AttachmentDescription> {
		self.desc.attachment_desc(num)
	}

	fn num_subpasses(&self) -> usize { self.desc.num_subpasses() }

	fn subpass_desc(&self, num: usize) -> Option<PassDescription> { self.desc.subpass_desc(num) }

	fn num_dependencies(&self) -> usize { self.desc.num_dependencies() }
...
where
	D: RenderPassDesc
{
	fn inner(&self) -> RenderPassSys { RenderPassSys(self.render_pass, PhantomData) }
}
...
unsafe impl<'a> VulkanObject for RenderPassSys<'a> {
	...
	fn internal_object(&self) -> vk::RenderPass { self.0 }
}
...
mod tests {
	use crate::{
		format::Format,
		framebuffer::{RenderPass, RenderPassCreationError}
	};

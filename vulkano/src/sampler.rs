...
impl Sampler {
	...
	pub fn new(
		...
		address_u: SamplerAddressMode, address_v: SamplerAddressMode,
		address_w: SamplerAddressMode, mip_lod_bias: f32, max_anisotropy: f32, min_lod: f32,
		max_lod: f32
	...
	fn new_impl(
		...
		address_u: SamplerAddressMode, address_v: SamplerAddressMode,
		address_w: SamplerAddressMode, mip_lod_bias: f32, max_anisotropy: f32, min_lod: f32,
		max_lod: f32, compare: Option<Compare>
	) -> Result<Arc<Sampler>, SamplerCreationError> {
	...
	/// - Panics if multiple `ClampToBorder` values are passed and the border color is different.
	pub fn unnormalized(
		device: Arc<Device>, filter: Filter, address_u: UnnormalizedSamplerAddressMode,
	...
	pub fn compare_mode(&self) -> bool { self.compare_mode }

	/// Returns true if the sampler is unnormalized.
	pub fn is_unnormalized(&self) -> bool { self.unnormalized }
	...
	pub fn usable_with_int_formats(&self) -> bool { self.usable_with_int_formats }
	...
	pub fn usable_with_swizzling(&self) -> bool { self.usable_with_swizzling }
}
...
unsafe impl VulkanObject for Sampler {
	...
	fn internal_object(&self) -> vk::Sampler { self.sampler }
}

...
use crate::{instance::loader::LoadingError, Error, OomError};
...
impl fmt::Display for SupportedExtensionsError {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		match self {
			SupportedExtensionsError::LoadingError(e) => {
				write!(f, "Failed to load Vulkan shared library: {}", e)
			}
			SupportedExtensionsError::OomError(e) => write!(f, "Not enough memory available: {}", e)
		}
	}
}
impl error::Error for SupportedExtensionsError {
	fn source(&self) -> Option<&(dyn error::Error + 'static)> {
		match self {
			SupportedExtensionsError::LoadingError(e) => e.source(),
			SupportedExtensionsError::OomError(e) => e.source()
		}
	}
}

impl From<OomError> for SupportedExtensionsError {
	fn from(err: OomError) -> SupportedExtensionsError { SupportedExtensionsError::OomError(err) }
}
...
impl From<Error> for SupportedExtensionsError {
	fn from(err: Error) -> SupportedExtensionsError {
		match err {
			Error::OutOfHostMemory => SupportedExtensionsError::OomError(OomError::from(err)),
			Error::OutOfDeviceMemory => SupportedExtensionsError::OomError(OomError::from(err)),

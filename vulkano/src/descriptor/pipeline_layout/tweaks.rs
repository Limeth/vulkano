...
use crate::descriptor::{
	descriptor::{DescriptorBufferDesc, DescriptorDesc, DescriptorDescTy},
	...
	T: PipelineLayoutDesc
{
		...
		I: IntoIterator<Item = (usize, usize)>
	{
		...
		for &(set, binding) in &dynamic_buffers {
			debug_assert!(
				inner.descriptor(set, binding).map_or(false, |desc| match desc.ty {
	...
	T: PipelineLayoutDesc
{
	...
	fn descriptor(&self, set: usize, binding: usize) -> Option<DescriptorDesc> {
		self.inner.descriptor(set, binding).map(|desc| match desc.ty {

...
impl RuntimePipelineDesc {
		...
		TPushConstsIter: IntoIterator<Item = PipelineLayoutDescPcRange>
	{
		...
		Ok(RuntimePipelineDesc { descriptors, push_constants })
	}
}

unsafe impl PipelineLayoutDesc for RuntimePipelineDesc {
	fn num_sets(&self) -> usize { self.descriptors.len() }
...
mod tests {
	...
	#[test]
	fn pc_conflict() {
		let range1 = PipelineLayoutDescPcRange { offset: 0, size: 8, stages: ShaderStages::all() };

		let range2 = PipelineLayoutDescPcRange { offset: 4, size: 8, stages: ShaderStages::all() };

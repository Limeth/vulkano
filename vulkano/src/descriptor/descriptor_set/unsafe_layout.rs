...
use crate::{
	descriptor::{descriptor::DescriptorDesc, descriptor_set::DescriptorsCount},
...
impl UnsafeDescriptorSetLayout {
	...
	pub fn new<I>(
		device: Arc<Device>, descriptors: I
		...
		I: IntoIterator<Item = Option<DescriptorDesc>>
	{
		...
		Ok(UnsafeDescriptorSetLayout { layout, device, descriptors_count })
	}

	/// Returns the number of descriptors of each type.
	pub fn descriptors_count(&self) -> &DescriptorsCount { &self.descriptors_count }
}
...
unsafe impl VulkanObject for UnsafeDescriptorSetLayout {
	...
	fn internal_object(&self) -> vk::DescriptorSetLayout { self.layout }
}
...
mod tests {
	use crate::descriptor::{
		...
		descriptor_set::{DescriptorsCount, UnsafeDescriptorSetLayout}
	};
	use std::iter;
	...
	fn basic_create() {
		...
		assert_eq!(
			...
			&DescriptorsCount { uniform_buffer: 1, ..DescriptorsCount::zero() }
		);
	}
}

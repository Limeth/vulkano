...
use crate::{
	buffer::{BufferAccess, BufferViewRef},
	descriptor::{
		descriptor::{
			DescriptorDesc,
			DescriptorDescTy,
	...
	R: PersistentDescriptorSetResources
{
	...
	fn num_buffers(&self) -> usize { self.resources.num_buffers() }

	fn buffer(&self, index: usize) -> Option<(&BufferAccess, u32)> { self.resources.buffer(index) }

	fn num_images(&self) -> usize { self.resources.num_images() }
	...
	L: PipelineLayoutAbstract
{
	...
	{
		assert_eq!(self.layout.device().internal_object(), pool.device().internal_object());
	...
	pub fn enter_array(
		...
		Ok(PersistentDescriptorSetBuilderArray { builder: self, desc, array_element: 0 })
	}
	...
	where
		T: ImageViewAccess
	{
		self.enter_array()?.add_sampled_image(image_view, sampler)?.leave_array()
	}
	...
	L: PipelineLayoutAbstract
{
		...
		T: ImageViewAccess
	{
		...
		let desc =
			match self.builder.layout.descriptor(self.builder.set_id, self.builder.binding_id) {
				Some(d) => d,
				None => return Err(PersistentDescriptorSetError::EmptyExpected)
			};
	...
	> {
		...
		Ok(PersistentDescriptorSetBuilderArray {
			builder: PersistentDescriptorSetBuilder {
				...
				binding_id: self.builder.binding_id,
				writes: self.builder.writes,
				resources: (self.builder.resources, PersistentDescriptorSetSampler { sampler })
			},
			desc: self.desc,
			array_element: self.array_element + 1
		})
...
fn image_match_desc<I>(
	image_view: &I, desc: &DescriptorImageDesc
...
unsafe impl PersistentDescriptorSetResources for () {
	fn num_buffers(&self) -> usize { 0 }

	fn buffer(&self, _: usize) -> Option<(&BufferAccess, u32)> { None }

	fn num_images(&self) -> usize { 0 }

	fn image(&self, _: usize) -> Option<(&ImageViewAccess, u32)> { None }
}
...
where
	...
	B: BufferAccess
{
	fn num_buffers(&self) -> usize { self.0.num_buffers() + 1 }
	...
	fn num_images(&self) -> usize { self.0.num_images() }

	fn image(&self, index: usize) -> Option<(&ImageViewAccess, u32)> { self.0.image(index) }
}
...
where
	...
	I: ImageViewAccess
{
	fn num_buffers(&self) -> usize { self.0.num_buffers() }

	fn buffer(&self, index: usize) -> Option<(&BufferAccess, u32)> { self.0.buffer(index) }

	fn num_images(&self) -> usize { self.0.num_images() + 1 }

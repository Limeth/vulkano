...
impl FixedSizeDescriptorSetsPool {
	...
	pub fn new(layout: Arc<PipelineLayout>, set_id: usize) -> FixedSizeDescriptorSetsPool {
		...
		FixedSizeDescriptorSetsPool {
			...
			set_id,
			set_layout,
			pool: LocalPool { device, next_capacity: 3, current_pool: None }
		}
	}
	...
	R: PersistentDescriptorSetResources
{
	...
	fn num_buffers(&self) -> usize { self.inner.num_buffers() }

	fn buffer(&self, index: usize) -> Option<(&BufferAccess, u32)> { self.inner.buffer(index) }

	fn num_images(&self) -> usize { self.inner.num_images() }
...
unsafe impl DescriptorPool for LocalPool {
	...
	fn alloc(&mut self, layout: &UnsafeDescriptorSetLayout) -> Result<Self::Alloc, OomError> {
		loop {
			...
			self.current_pool =
				Some(Arc::new(LocalPoolInner { actual_pool: new_pool, reserve: alloc }));
		}
	}
}
...
impl DescriptorPoolAlloc for LocalPoolAlloc {
	fn inner(&self) -> &UnsafeDescriptorSet { self.actual_alloc.as_ref().unwrap() }

	fn inner_mut(&mut self) -> &mut UnsafeDescriptorSet { self.actual_alloc.as_mut().unwrap() }
}
...
impl<'a, R> FixedSizeDescriptorSetBuilder<'a, R> {
	...
	where
		T: BufferAccess
	{
		Ok(FixedSizeDescriptorSetBuilder { pool: self.pool, inner: self.inner.add_buffer(buffer)? })
	}

...
use std::{cmp, error, fmt, mem, ops, ptr, sync::Arc, vec::IntoIter as VecIntoIter};

use crate::{
	buffer::{BufferAccess, BufferInner, BufferView},
	descriptor::{descriptor::DescriptorType, descriptor_set::UnsafeDescriptorSetLayout},
	device::{Device, DeviceOwned},
	image::ImageViewAccess,
	sampler::Sampler
};

use crate::{check_errors, OomError, VulkanObject};
use vk_sys as vk;
...
impl UnsafeDescriptorPool {
	...
	pub fn new(
		device: Arc<Device>, count: &DescriptorsCount, max_sets: u32, free_descriptor_set_bit: bool
		...
		macro_rules! elem {
			($field:ident, $ty:expr) => {
				if count.$field >= 1 {
					pool_sizes
						.push(vk::DescriptorPoolSize { ty: $ty, descriptorCount: count.$field });
					}
			};
		}
		...
		Ok(UnsafeDescriptorPool { pool, device: device.clone() })
	}
	...
	unsafe fn alloc_impl(
		&mut self, layouts: &SmallVec<[vk::DescriptorSetLayout; 8]>
	) -> Result<UnsafeDescriptorPoolAllocIter, DescriptorPoolAllocError> {
		let num = layouts.len();
		...
		match ret {
			vk::ERROR_OUT_OF_HOST_MEMORY => return Err(DescriptorPoolAllocError::OutOfHostMemory),
			...
			c if (c as i32) < 0 => return Err(DescriptorPoolAllocError::FragmentedPool),
			_ => ()
		};
	...
	unsafe fn free_impl(
		&mut self, sets: &SmallVec<[vk::DescriptorSet; 8]>
	) -> Result<(), OomError> {
		let vk = self.device.pointers();
...
impl Iterator for UnsafeDescriptorPoolAllocIter {
	...
	fn size_hint(&self) -> (usize, Option<usize>) { self.sets.size_hint() }
}
...
unsafe impl VulkanObject for UnsafeDescriptorSet {
	...
	fn internal_object(&self) -> vk::DescriptorSet { self.set }
}
...
impl DescriptorWrite {
	...
	pub fn uniform_texel_buffer<'a, F, B>(
		binding: u32, array_element: u32, view: &BufferView<F, B>
	) -> DescriptorWrite
	where
		B: BufferAccess
	{
		...
		B: BufferAccess
	{
		...
		debug_assert_eq!(
			offset
				% buffer.device().physical_device().limits().min_uniform_buffer_offset_alignment()
					as usize,
			0
		);
		debug_assert!(
			size <= buffer.device().physical_device().limits().max_uniform_buffer_range() as usize
		);
		...
		B: BufferAccess
	{
		...
		debug_assert_eq!(
			offset
				% buffer.device().physical_device().limits().min_storage_buffer_offset_alignment()
					as usize,
			0
		);
		debug_assert!(
			size <= buffer.device().physical_device().limits().max_storage_buffer_range() as usize
		);
	...
	pub unsafe fn dynamic_uniform_buffer<B>(
		binding: u32, array_element: u32, buffer: &B
	) -> DescriptorWrite
	where
		B: BufferAccess
	{
...
mod tests {
	use crate::descriptor::{
		...
		descriptor_set::{DescriptorsCount, UnsafeDescriptorPool, UnsafeDescriptorSetLayout}
	};
	use std::iter;
	...
	fn pool_create() {
		...
		let desc = DescriptorsCount { uniform_buffer: 1, ..DescriptorsCount::zero() };
	...
	fn basic_alloc() {
		...
		let desc = DescriptorsCount { uniform_buffer: 10, ..DescriptorsCount::zero() };

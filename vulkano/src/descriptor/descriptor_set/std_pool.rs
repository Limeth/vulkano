...
impl StdDescriptorPool {
	...
	pub fn new(device: Arc<Device>) -> StdDescriptorPool {
		StdDescriptorPool { device, pools: Mutex::new(Vec::new()) }
	}
}
...
unsafe impl DescriptorPool for Arc<StdDescriptorPool> {
	...
	fn alloc(
		&mut self, layout: &UnsafeDescriptorSetLayout
...
impl DescriptorPoolAlloc for StdDescriptorPoolAlloc {
	fn inner(&self) -> &UnsafeDescriptorSet { self.set.as_ref().unwrap() }

	fn inner_mut(&mut self) -> &mut UnsafeDescriptorSet { self.set.as_mut().unwrap() }
}
...
mod tests {
	use crate::descriptor::{
		descriptor::{DescriptorDesc, DescriptorDescTy, ShaderStages},
		descriptor_set::{DescriptorPool, StdDescriptorPool, UnsafeDescriptorSetLayout}
	};

	...
	F: GpuFuture
{
	...
	fn flush_impl(
		&self, state: &mut MutexGuard<FenceSignalFutureState<F>>
	) -> Result<(), FlushError> {
		unsafe {
			...
			let result = match previous.build_submission()? {
				...
				SubmitAnyBuilder::QueuePresent(present) => {
					let intermediary_result =
						if partially_flushed { Ok(()) } else { present.submit(&queue) };
...
where
	F: GpuFuture
{
	fn cleanup_finished(&mut self) { self.cleanup_finished_impl() }

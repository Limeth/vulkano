	...
	D: SafeDeref<Target = Device>
{
	...
	pub fn from_pool(device: D) -> Result<Semaphore<D>, OomError> {
		...
		match maybe_raw_sem {
			Some(raw_sem) => Ok(Semaphore { device, semaphore: raw_sem, must_put_in_pool: true }),
			None => {
	...
	fn alloc_impl(device: D, must_put_in_pool: bool) -> Result<Semaphore<D>, OomError> {
		...
		Ok(Semaphore { device, semaphore, must_put_in_pool })
	}
}
	...
	D: SafeDeref<Target = Device>
{
	...
	fn internal_object(&self) -> vk::Semaphore { self.semaphore }
}

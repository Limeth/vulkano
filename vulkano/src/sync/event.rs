...
impl Event {
	...
	pub fn from_pool(device: Arc<Device>) -> Result<Event, OomError> {
		...
		match maybe_raw_event {
			Some(raw_event) => {
				...
				Ok(Event { event: raw_event, device, must_put_in_pool: true })
			}
			None => {
	...
	fn alloc_impl(device: Arc<Device>, must_put_in_pool: bool) -> Result<Event, OomError> {
		...
		Ok(Event { device, event, must_put_in_pool })
	}
	...
	pub fn set(&mut self) { self.set_raw().unwrap(); }
	...
	///
	/// - Panics if the device or host ran out of memory.
	pub fn reset(&mut self) { self.reset_raw().unwrap(); }
}
...
unsafe impl VulkanObject for Event {
	...
	fn internal_object(&self) -> vk::Event { self.event }
}

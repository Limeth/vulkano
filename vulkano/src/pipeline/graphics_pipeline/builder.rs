...
use crate::{
	...
	pipeline::{
		blend::{AttachmentBlend, AttachmentsBlend, Blend, LogicOp},
		depth_stencil::{Compare, DepthBounds, DepthStencil},
		graphics_pipeline::{
			GraphicsPipeline,
			GraphicsPipelineCreationError,
			Inner as GraphicsPipelineInner
		},
		input_assembly::PrimitiveTopology,
		raster::{CullMode, DepthBiasControl, FrontFace, PolygonMode, Rasterization},
		shader::{
		...
		vertex::{BufferlessDefinition, SingleBufferDefinition, VertexDefinition},
		viewport::{Scissor, Viewport, ViewportsState}
	}
};
...
use crate::{
	...
	framebuffer::RenderPassSubpassInterface,
	VulkanObject
};
use vk_sys as vk;
	...
	Rp: RenderPassAbstract + RenderPassSubpassInterface<Fs::OutputDefinition>
{
	...
	pub fn build(
		...
		GraphicsPipeline<Vdef, Rp>,
		GraphicsPipelineCreationError
	> {
		self.with_auto_layout(device, &[])
	}

	pub fn construct_layout_desc(&self, dynamic_buffers: &[(usize, usize)]) -> Result<PipelineLayoutDescAggregation, GraphicsPipelineCreationError> {
		Ok(if let Some(ref tess) = self.tessellation {
			if let Some(ref gs) = self.geometry_shader {
				...
				PipelineLayoutDescTweaks::new(self.vertex_shader.as_ref().unwrap().0.layout().clone()
					...
					dynamic_buffers.into_iter().cloned()).aggregate()
			} else {
				if let Err(err) = tess.tessellation_control_shader
					.0
					.input()
	...
	pub fn with_pipeline_layout(
		...
		let stages = {
			...
			if let Some(ref tess) = self.tessellation {
				...
				stages.push(vk::PipelineShaderStageCreateInfo {
					...
					module: tess.tessellation_evaluation_shader.0.module().internal_object(),
		...
		let (binding_descriptions, attribute_descriptions) = {
			let (buffers_iter, attribs_iter) =
				self.vertex_input.definition(self.vertex_shader.as_ref().unwrap().0.input())?;

			let mut binding_descriptions = SmallVec::<[_; 8]>::new();
			for (num, stride, rate) in buffers_iter {
				if stride
					> device.physical_device().limits().max_vertex_input_binding_stride() as usize
				{
					return Err(GraphicsPipelineCreationError::MaxVertexInputBindingStrideExceeded {
						binding: num as usize,
						max: device.physical_device().limits().max_vertex_input_binding_stride()
							as usize,
						obtained: stride
					})
				}
			...
			for (loc, binding, info) in attribs_iter {
				...
				if info.offset
					> device.physical_device().limits().max_vertex_input_attribute_offset() as usize
				{
					return Err(
		...
		if binding_descriptions.len()
			> device.physical_device().limits().max_vertex_input_bindings() as usize
		{
		...
		if attribute_descriptions.len()
			> device.physical_device().limits().max_vertex_input_attributes() as usize
		{
		...
		let tessellation = match self.input_assembly_topology {
			PrimitiveTopology::PatchList { vertices_per_patch } => {
				...
				if vertices_per_patch
					> device.physical_device().limits().max_tessellation_patch_size()
				{
		...
		for vp in vp_vp.iter() {
			...
			{
				return Err(GraphicsPipelineCreationError::MaxViewporImageDimensionsExceeded)
			}
		...
		let rasterization = vk::PipelineRasterizationStateCreateInfo {
			...
			pNext: ptr::null(),
			flags: 0, // reserved
			depthClampEnable: if self.raster.depth_clamp { vk::TRUE } else { vk::FALSE },
		...
		self.multisample.rasterizationSamples =
			self.render_pass.as_ref().unwrap().num_samples().unwrap_or(1);
		...
		let depth_stencil = {
			...
			vk::PipelineDepthStencilStateCreateInfo {
				...
				depthWriteEnable: if self.depth_stencil.depth_write { vk::TRUE } else { vk::FALSE },
				...
				front: vk::StencilOpState {
					...
					compareMask: self.depth_stencil.stencil_front.compare_mask.unwrap_or(u32::MAX),
					writeMask: self.depth_stencil.stencil_front.write_mask.unwrap_or(u32::MAX),
				...
				back: vk::StencilOpState {
					...
					compareMask: self.depth_stencil.stencil_back.compare_mask.unwrap_or(u32::MAX),
					writeMask: self.depth_stencil.stencil_back.write_mask.unwrap_or(u32::MAX),
		...
		Ok(GraphicsPipeline {
			inner: GraphicsPipelineInner { device: device.clone(), pipeline },
	...
	GraphicsPipelineBuilder<Vdef, Vs, Vss, Tcs, Tcss, Tes, Tess, Gs, Gss, Fs, Fss, Rp>
{
	...
	pub fn tessellation_shaders<Tcs2, Tcss2, Tes2, Tess2>(
		self, tessellation_control_shader: Tcs2, tessellation_control_shader_spec_constants: Tcss2,

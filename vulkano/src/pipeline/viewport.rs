...
pub struct Viewport {
	...
	/// ImageDimensions in pixels of the viewport.
...
pub struct Scissor {
	...
	/// ImageDimensions in pixels of the box.
...
impl Scissor {
	...
	pub fn irrelevant() -> Scissor {
		Scissor { origin: [0, 0], dimensions: [0x7fffffff, 0x7fffffff] }
	}

	pub(crate) fn into_vulkan_rect(self) -> vk::Rect2D {
		vk::Rect2D {
			offset: vk::Offset2D { x: self.origin[0], y: self.origin[1] },
			extent: vk::Extent2D { width: self.dimensions[0], height: self.dimensions[1] }
		}
	}
}

impl Default for Scissor {
	fn default() -> Scissor { Scissor::irrelevant() }
}

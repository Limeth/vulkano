...
use std::{marker::PhantomData, mem, sync::Arc, vec::IntoIter as VecIntoIter};
	...
	fn definition(
		...
		let attrib = {
			...
			for e in interface.elements() {
				...
				if !infos.ty.matches(infos.array_size, e.format, e.location.end - e.location.start)
				{
				...
				for loc in e.location.clone() {
					attribs.push((loc, buf_offset, AttributeInfo { offset, format: e.format }));
...
{
	fn decode(
		&self, source: Vec<Arc<BufferAccess + Send + Sync>>
	) -> (Vec<Box<BufferAccess + Send + Sync>>, usize, usize) {
		// FIXME: safety
		assert_eq!(source.len(), 2);
		let vertices =
			[source[0].size() / mem::size_of::<T>(), source[1].size() / mem::size_of::<U>()]
				.iter()
				.cloned()
				.min()
				.unwrap();

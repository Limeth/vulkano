...
impl PipelineCache {
	...
	pub unsafe fn with_data(
		device: Arc<Device>, initial_data: &[u8]
	...
	unsafe fn new_impl(
		device: Arc<Device>, initial_data: Option<&[u8]>
	) -> Result<Arc<PipelineCache>, OomError> {
		...
		Ok(Arc::new(PipelineCache { device: device.clone(), cache }))
	}
...
unsafe impl VulkanObject for PipelineCache {
	...
	fn internal_object(&self) -> vk::PipelineCache { self.cache }
}

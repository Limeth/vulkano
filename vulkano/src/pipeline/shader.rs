...
use crate::{
	descriptor::pipeline_layout::{EmptyPipelineDesc, PipelineLayoutDesc},
	format::Format,
	pipeline::input_assembly::PrimitiveTopology
};

use crate::{check_errors, device::Device, OomError, VulkanObject};
use vk_sys as vk;
...
impl ShaderModule {
	...
	///   this function either.
	unsafe fn from_ptr(
		device: Arc<Device>, spirv: *const u32, spirv_len: usize
	) -> Result<Arc<ShaderModule>, OomError> {
		let module = {
		...
		Ok(Arc::new(ShaderModule { module, device }))
	}
	...
	pub unsafe fn graphics_entry_point<'a, S, I, O, L>(
		&'a self, name: &'a CStr, input: I, output: O, layout: L, ty: GraphicsShaderType
	) -> GraphicsEntryPoint<'a, S, I, O, L> {
		GraphicsEntryPoint { module: self, name, input, output, layout, ty, marker: PhantomData }
	}
	...
	pub unsafe fn compute_entry_point<'a, S, L>(
		&'a self, name: &'a CStr, layout: L
	) -> ComputeEntryPoint<'a, S, L> {
		ComputeEntryPoint { module: self, name, layout, marker: PhantomData }
	}
}
...
unsafe impl VulkanObject for ShaderModule {
	...
	fn internal_object(&self) -> vk::ShaderModule { self.module }
}
	...
	S: SpecializationConstants
{
	...
	fn module(&self) -> &ShaderModule { self.module }

	fn name(&self) -> &CStr { self.name }

	fn layout(&self) -> &L { &self.layout }
}
	...
	S: SpecializationConstants
{
	...
	fn input(&self) -> &I { &self.input }

	fn output(&self) -> &O { &self.output }

	fn ty(&self) -> GraphicsShaderType { self.ty }
}
...
unsafe impl EntryPointAbstract for EmptyEntryPointDummy {
	...
	fn module(&self) -> &ShaderModule { unreachable!() }

	fn name(&self) -> &CStr { unreachable!() }

	fn layout(&self) -> &EmptyPipelineDesc { unreachable!() }
}
...
unsafe impl GraphicsEntryPointAbstract for EmptyEntryPointDummy {
	...
	fn input(&self) -> &EmptyShaderInterfaceDef { unreachable!() }

	fn output(&self) -> &EmptyShaderInterfaceDef { unreachable!() }

	fn ty(&self) -> GraphicsShaderType { unreachable!() }
}
...
unsafe impl ShaderInterfaceDef for EmptyShaderInterfaceDef {
	...
	fn elements(&self) -> Self::Iter { iter::empty() }
}
...
/// 			SpecializationMapEntry { constant_id: 0, offset: 0, size: 4 },
/// 			SpecializationMapEntry { constant_id: 1, offset: 4, size: 4 },
/// 			SpecializationMapEntry { constant_id: 2, offset: 8, size: 4 }
/// 		];
///
/// 		&DESCRIPTORS
/// 		}

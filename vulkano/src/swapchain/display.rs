...
use std::{ffi::CStr, ptr, sync::Arc, vec::IntoIter};
...
use crate::{
	...
	swapchain::{capabilities, SupportedSurfaceTransforms},
	OomError,
	VulkanObject
};
...
impl Display {
	...
	pub fn plane_reorder_possible(&self) -> bool { self.properties.planeReorderPossible != 0 }

	/// Returns true if TODO.
	pub fn persistent_content(&self) -> bool { self.properties.persistentContent != 0 }
	...
	pub fn display_modes(&self) -> IntoIter<DisplayMode> { self.display_modes_raw().unwrap() }
}
...
unsafe impl VulkanObject for Display {
	...
	fn internal_object(&self) -> vk::DisplayKHR { self.properties.display }
}
...
impl DisplayMode {
	...
	pub fn refresh_rate(&self) -> u32 { self.parameters.refreshRate }
}
...
unsafe impl VulkanObject for DisplayMode {
	...
	fn internal_object(&self) -> vk::DisplayModeKHR { self.display_mode }
}

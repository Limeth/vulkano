...
impl RectangleLayer {
	...
	pub fn is_compatible_with<W>(&self, swapchain: &Swapchain<W>) -> bool {
		...
		self.offset[0] as u32 + self.extent[0] <= swapchain.dimensions()[0].get()
			&& self.offset[1] as u32 + self.extent[1] <= swapchain.dimensions()[1].get()
			&& self.layer < swapchain.layers().get()
	}

	pub(crate) fn to_vk(&self) -> vk::RectLayerKHR {
		vk::RectLayerKHR {
			offset: vk::Offset2D { x: self.offset[0], y: self.offset[1] },

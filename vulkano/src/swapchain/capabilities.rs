...
use crate::{format::Format, image::ImageUsage};
...
pub enum PresentMode {
	...
	///
	/// This is the equivalent of OpenGL's `SwapInterval` with a value of -1.
	Relaxed = vk::PRESENT_MODE_FIFO_RELAXED_KHR /* TODO: These can't be enabled yet because they have to be used with shared present surfaces
...
impl SupportedPresentModes {
	...
	pub fn iter(&self) -> SupportedPresentModesIter { SupportedPresentModesIter(self.clone()) }
}
...
impl SupportedCompositeAlpha {
	...
	pub fn iter(&self) -> SupportedCompositeAlphaIter { SupportedCompositeAlphaIter(self.clone()) }
}
...
impl Default for SurfaceTransform {
	fn default() -> SurfaceTransform { SurfaceTransform::Identity }
}

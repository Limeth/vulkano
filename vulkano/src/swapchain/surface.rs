...
impl<W> Surface<W> {
	...
	pub unsafe fn from_raw_surface(
		instance: Arc<Instance>, surface: vk::SurfaceKHR, win: W
	) -> Surface<W> {
		Surface { window: win, instance, surface, has_swapchain: AtomicBool::new(false) }
	}
	...
	pub fn from_display_mode(
		display_mode: &DisplayMode, plane: &DisplayPlane
	...
	pub unsafe fn from_hwnd<T, U>(
		instance: Arc<Instance>, hinstance: *const T, hwnd: *const U, win: W
	...
	pub unsafe fn from_xcb<C>(
		instance: Arc<Instance>, connection: *const C, window: u32, win: W
	) -> Result<Arc<Surface<W>>, SurfaceCreationError> {
		let vk = instance.pointers();
	...
	pub unsafe fn from_xlib<D>(
		instance: Arc<Instance>, display: *const D, window: c_ulong, win: W
	) -> Result<Arc<Surface<W>>, SurfaceCreationError> {
		let vk = instance.pointers();
	...
	pub unsafe fn from_wayland<D, S>(
		instance: Arc<Instance>, display: *const D, surface: *const S, win: W
	) -> Result<Arc<Surface<W>>, SurfaceCreationError> {
		let vk = instance.pointers();
	...
	pub unsafe fn from_anativewindow<T>(
		instance: Arc<Instance>, window: *const T, win: W
	) -> Result<Arc<Surface<W>>, SurfaceCreationError> {
		let vk = instance.pointers();
	...
	pub unsafe fn from_ios_moltenvk<T>(
		instance: Arc<Instance>, view: *const T, win: W
	) -> Result<Arc<Surface<W>>, SurfaceCreationError> {
		let vk = instance.pointers();
	...
	pub fn window(&self) -> &W { &self.window }
...
unsafe impl<W> SurfaceSwapchainLock for Surface<W> {
	fn flag(&self) -> &AtomicBool { &self.has_swapchain }
}
...
unsafe impl<W> VulkanObject for Surface<W> {
	...
	fn internal_object(&self) -> vk::SurfaceKHR { self.surface }
}
...
impl fmt::Display for SurfaceCreationError {
	fn fmt(&self, f: &mut fmt::Formatter) -> Result<(), fmt::Error> {
		match self {
			SurfaceCreationError::OomError(e) => e.fmt(f),
			SurfaceCreationError::MissingExtension { name } => {
				write!(f, "The extension `{}` was not enabled", name)
			}
		}
	}
}
impl error::Error for SurfaceCreationError {
	fn source(&self) -> Option<&(dyn error::Error + 'static)> {
		match self {
			SurfaceCreationError::OomError(e) => e.source(),
			_ => None
		}
	}
}
...
impl From<OomError> for SurfaceCreationError {
	fn from(err: OomError) -> SurfaceCreationError { SurfaceCreationError::OomError(err) }
}
...
impl fmt::Display for CapabilitiesError {
	fn fmt(&self, f: &mut fmt::Formatter) -> Result<(), fmt::Error> {
		match self {
			CapabilitiesError::OomError(e) => e.fmt(f),
			CapabilitiesError::SurfaceLost => write!(f, "The surface is no longer valid")
		}
	}
}
impl error::Error for CapabilitiesError {
	fn source(&self) -> Option<&(dyn error::Error + 'static)> {
		match self {
			CapabilitiesError::OomError(e) => e.source(),
			_ => None
		}
	}
}
...
impl From<OomError> for CapabilitiesError {
	fn from(err: OomError) -> CapabilitiesError { CapabilitiesError::OomError(err) }
}

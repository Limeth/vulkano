...
use crate::{check_errors, Error, OomError, SynchronizedVulkanObject, VulkanHandle, VulkanObject};
use vk_sys as vk;

pub use crate::features::Features;
pub use extensions::{DeviceExtensions, RawDeviceExtensions};
mod extensions;
...
impl Device {
	...
	pub fn new<'a, I, Ext>(
		phys: PhysicalDevice, requested_features: &Features, extensions: Ext, queue_families: I
		...
		Ext: Into<RawDeviceExtensions>
	{
		...
		let extensions_list =
			extensions.iter().map(|extension| extension.as_ptr()).collect::<SmallVec<[_; 16]>>();
		...
		let output_queues =
			QueuesIter { next_queue: 0, device: device.clone(), families_and_ids: output_queues };

		Ok((device, output_queues))
	}

	/// Grants access to the pointers to the Vulkan functions of the device.
	pub(crate) fn pointers(&self) -> &vk::DevicePointers { &self.vk }
	...
	pub fn enabled_features(&self) -> &Features { &self.features }

	/// Returns the list of extensions that have been loaded.
	pub fn loaded_extensions(&self) -> &DeviceExtensions { &self.extensions }
	...
	pub(crate) fn allocation_count(&self) -> &Mutex<u32> { &self.allocation_count }

	pub(crate) fn fence_pool(&self) -> &Mutex<Vec<vk::Fence>> { &self.fence_pool }

	pub(crate) fn semaphore_pool(&self) -> &Mutex<Vec<vk::Semaphore>> { &self.semaphore_pool }

	pub(crate) fn event_pool(&self) -> &Mutex<Vec<vk::Event>> { &self.event_pool }
	...
	pub unsafe fn set_object_name_raw(
		&self, ty: vk::DebugReportObjectTypeEXT, object: u64, name: &CStr
	) -> Result<(), OomError> {
...
unsafe impl VulkanObject for Device {
	...
	fn internal_object(&self) -> vk::Device { self.device }
}
...
impl Queue {
	...
	pub fn id_within_family(&self) -> u32 { self.id }
...
unsafe impl SynchronizedVulkanObject for Queue {
	...
	fn internal_object_guard(&self) -> MutexGuard<vk::Queue> { self.queue.lock().unwrap() }
}

#[cfg(test)]
mod tests {
	use crate::{
		device::{Device, DeviceCreationError, DeviceExtensions},
		features::Features,
		instance
	};
	use std::sync::Arc;

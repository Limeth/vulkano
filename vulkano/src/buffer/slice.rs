...
use std::{marker::PhantomData, mem, ops::Range, sync::Arc};
...
impl<T: ?Sized, B> BufferSlice<T, B> {
		...
		B: TypedBufferAccess<Content = T>
	{
		...
		BufferSlice { marker: PhantomData, resource: r, offset: 0, size }
	}

	/// Returns the buffer that this slice belongs to.
	pub fn buffer(&self) -> &B { &self.resource }
	...
	/// let vertex_slice: BufferSlice<[VertexImpl], Arc<ImmutableBuffer<[u8]>>> =
	/// 	unsafe { blob_slice.reinterpret::<[VertexImpl]>() };
	/// ```
	///
	/// # Safety
	///
	...
	B: BufferAccess
{
	fn inner(&self) -> BufferInner {
		...
		BufferInner { buffer: inner.buffer, offset: inner.offset + self.offset }
	}
	...
	unsafe fn increase_gpu_lock(&self) { self.resource.increase_gpu_lock() }

	unsafe fn unlock(&self) { self.resource.unlock() }
}
...
where
	B: DeviceOwned
{
	fn device(&self) -> &Arc<Device> { self.resource.device() }
}

impl<T, B> From<BufferSlice<T, B>> for BufferSlice<[T], B> {
	fn from(r: BufferSlice<T, B>) -> BufferSlice<[T], B> {
		BufferSlice { marker: PhantomData, resource: r.resource, offset: r.offset, size: r.size }
	}
}

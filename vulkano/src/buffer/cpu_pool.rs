	...
	A: MemoryPool
{
	...
	pub fn reserve(&self, capacity: usize) -> Result<(), DeviceMemoryAllocError> {
		...
		match *cur_buf {
			Some(ref buf) if buf.capacity >= capacity => return Ok(()),
			_ => ()
		};
	...
	fn reset_buf(
		&self, cur_buf_mutex: &mut MutexGuard<Option<Arc<ActualBuffer<A>>>>, capacity: usize
	...
	//
	fn try_next_impl<I>(
		&self, cur_buf_mutex: &mut MutexGuard<Option<Arc<ActualBuffer<A>>>>, mut data: I
	...
	A: MemoryPool
{
	fn clone(&self) -> CpuBufferPoolChunk<T, A> {
		...
		chunk.num_cpu_accesses =
			chunk.num_cpu_accesses.checked_add(1).expect("Overflow in CPU accesses");
	...
	A: MemoryPool
{
	...
	unsafe fn increase_gpu_lock(&self) {
		...
		chunk.num_gpu_accesses =
			chunk.num_gpu_accesses.checked_add(1).expect("Overflow in GPU usages");
	}
...
where
	A: MemoryPool
{
	fn device(&self) -> &Arc<Device> { self.buffer.inner.device() }
}
...
where
	A: MemoryPool
{
	fn inner(&self) -> BufferInner { self.chunk.inner() }
	...
	unsafe fn increase_gpu_lock(&self) { self.chunk.increase_gpu_lock() }

	unsafe fn unlock(&self) { self.chunk.unlock() }
}
...
where
	A: MemoryPool
{
	fn device(&self) -> &Arc<Device> { self.chunk.buffer.inner.device() }
}

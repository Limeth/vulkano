...
impl UnsafeBuffer {
	...
	pub unsafe fn new<'a, I>(
		device: Arc<Device>, size: usize, usage: BufferUsage, sharing: Sharing<I>,
		sparse: SparseLevel
		...
		I: Iterator<Item = u32>
	{
		...
		assert!(usage_bits != 0, "Can't create buffer with empty BufferUsage");
		...
		let mem_reqs = {
			fn align(val: usize, al: usize) -> usize { al * (1 + (val - 1) / al) }
			...
			if usage.uniform_texel_buffer || usage.storage_texel_buffer {
				output.alignment =
					align(output.alignment, limits.min_texel_buffer_offset_alignment() as usize);
			}

			if usage.storage_buffer {
				output.alignment =
					align(output.alignment, limits.min_storage_buffer_offset_alignment() as usize);
			}

			if usage.uniform_buffer {
				output.alignment =
					align(output.alignment, limits.min_uniform_buffer_offset_alignment() as usize);
			}
		...
		let obj =
			UnsafeBuffer { buffer, device: device.clone(), size: size as usize, usage: usage_bits };
	...
	pub fn key(&self) -> u64 { self.buffer }
}
...
unsafe impl VulkanObject for UnsafeBuffer {
	...
	fn internal_object(&self) -> vk::Buffer { self.buffer }
}
...
impl SparseLevel {
	pub fn none() -> SparseLevel {
		SparseLevel { sparse: false, sparse_residency: false, sparse_aliased: false }
	}
...
mod tests {
	...
	use super::{BufferCreationError, BufferUsage, SparseLevel, UnsafeBuffer};

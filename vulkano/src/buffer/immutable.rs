...
impl<T: ?Sized> ImmutableBuffer<T> {
		...
		T: 'static + Send + Sync
	{
		unsafe {
			...
			let actual_usage = BufferUsage { transfer_destination: true, ..usage };
...
impl<T> ImmutableBuffer<[T]> {
	...
	pub unsafe fn uninitialized_array(
		...
		(Arc<ImmutableBuffer<[T]>>, ImmutableBufferInitialization<[T]>),
		DeviceMemoryAllocError
	> {
		ImmutableBuffer::raw(
			device.clone(),
...
unsafe impl<T: ?Sized, A> BufferAccess for ImmutableBufferInitialization<T, A> {
	fn inner(&self) -> BufferInner { self.buffer.inner() }
	...
	unsafe fn unlock(&self) { self.buffer.initialized.store(true, Ordering::Relaxed); }
}
...
impl<T: ?Sized, A> Clone for ImmutableBufferInitialization<T, A> {
	fn clone(&self) -> ImmutableBufferInitialization<T, A> {
		ImmutableBufferInitialization { buffer: self.buffer.clone(), used: self.used.clone() }
	}
}
...
mod tests {
	use crate::{
		buffer::{cpu_access::CpuAccessibleBuffer, immutable::ImmutableBuffer, BufferUsage},

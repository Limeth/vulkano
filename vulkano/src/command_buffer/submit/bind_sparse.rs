...
use crate::{check_errors, Error, OomError, SynchronizedVulkanObject, VulkanObject};
use vk_sys as vk;
...
impl<'a> SubmitBindSparseBuilder<'a> {
	...
	pub fn new() -> SubmitBindSparseBuilder<'a> {
		SubmitBindSparseBuilder { infos: SmallVec::new(), fence: 0 }
	}
	...
	pub fn add(&mut self, builder: SubmitBindSparseBatchBuilder<'a>) { self.infos.push(builder); }
	...
	///
	/// ```
	/// use vulkano::{command_buffer::submit::SubmitBindSparseBuilder, sync::Fence};
	...
	/// 	}
	/// ```
	pub fn has_fence(&self) -> bool { self.fence != 0 }
	...
	/// error.
	pub fn merge(
		&mut self, other: SubmitBindSparseBuilder<'a>
...
impl<'a> SubmitBindSparseBatchBuilder<'a> {
	...
	pub fn num_signal_semaphores(&self) -> usize { self.signal_semaphores.len() }
...
impl<'a> SubmitBindSparseBufferBindBuilder<'a> {
	...
	pub unsafe fn add_bind(
		&mut self, offset: usize, size: usize, memory: &DeviceMemory, memory_offset: usize
	) {
...
impl<'a> SubmitBindSparseImageOpaqueBindBuilder<'a> {
	...
	pub unsafe fn add_bind(
		...
		self.binds.push(vk::SparseMemoryBind {
			...
			memory: memory.internal_object(),
			memoryOffset: memory_offset as vk::DeviceSize,
			flags: if bind_metadata { vk::SPARSE_MEMORY_BIND_METADATA_BIT } else { 0 }
		});
	}

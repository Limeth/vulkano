...
use crate::{
	...
	command_buffer::{
		...
		synced::{SyncCommandBuffer, SyncCommandBufferBuilder, SyncCommandBufferBuilderError},
		sys::{
			Flags,
			Kind,
			KindOcclusionQuery,
	...
	sampler::Filter,
	sync::{AccessCheckError, AccessFlagBits, GpuFuture, PipelineStages},
	OomError
};
...
impl AutoCommandBufferBuilder<StandardCommandPoolBuilder> {
	...
	pub fn secondary_compute(
		...
		let kind =
			Kind::secondary(KindOcclusionQuery::Forbidden, QueryPipelineStatisticFlags::none());
	...
	fn with_flags<R, F>(
		device: Arc<Device>, queue_family: QueueFamily, kind: Kind<R, F>, flags: Flags
	) -> Result<AutoCommandBufferBuilder<StandardCommandPoolBuilder>, OomError>
	where
		R: RenderPassAbstract + Clone + Send + Sync + 'static,
...
impl<P> AutoCommandBufferBuilder<P> {
		...
		P: CommandPoolBuilderAlloc
	{
		...
		let submit_state = match self.flags {
			Flags::None => SubmitState::ExclusiveUse { in_use: AtomicBool::new(false) },
		...
		Ok(AutoCommandBuffer { inner: self.inner.build()?, submit_state })
	}
	...
	pub fn begin_render_pass<F, C>(
		mut self, framebuffer: F, secondary: bool, clear_values: C
		...
		F: FramebufferAbstract + RenderPassDescClearValues<C> + Clone + Send + Sync + 'static
	{
		unsafe {
			...
			self.inner.begin_render_pass(framebuffer.clone(), contents, clear_values)?;
	...
	pub fn copy_image<S, D>(
		mut self, source: S, source_offset: [i32; 3], source_base_array_layer: u32,
	...
	pub fn clear_color_image_dimensions<I>(
		mut self, image: I, first_layer: u32, num_layers: u32, first_mipmap: u32, num_mipmaps: u32,
		color: ClearValue
	) -> Result<Self, ClearColorImageError>
	where
		I: ImageAccess + Send + Sync + 'static
	...
	pub fn copy_buffer<S, D, T>(
		mut self, source: S, destination: D
	...
	pub fn dispatch<Cp, S, Pc>(
		mut self, dimensions: [u32; 3], pipeline: Cp, sets: S, constants: Pc
	...
	pub fn draw<V, Gp, S, Pc>(
		mut self, pipeline: Gp, dynamic: &DynamicState, vertex_buffer: V, sets: S, constants: Pc
		...
		S: DescriptorSetsCollection
	{
		unsafe {
			...
			self.inner.draw(vb_infos.vertex_count as u32, vb_infos.instance_count as u32, 0, 0);
			Ok(self)
		}
	}
	...
	pub fn draw_indexed<V, Gp, S, Pc, Ib, I>(
		mut self, pipeline: Gp, dynamic: &DynamicState, vertex_buffer: V, index_buffer: Ib,
		sets: S, constants: Pc
	...
	pub fn draw_indirect<V, Gp, S, Pc, Ib>(
		mut self, pipeline: Gp, dynamic: &DynamicState, vertex_buffer: V, indirect_buffer: Ib,
		sets: S, constants: Pc
...
unsafe fn vertex_buffers<P>(
	destination: &mut SyncCommandBufferBuilder<P>, state_cacher: &mut StateCacher,
...
unsafe impl<P> CommandBuffer for AutoCommandBuffer<P> {
	...
	fn inner(&self) -> &UnsafeCommandBuffer<P> { self.inner.as_ref() }
	...
	($name: ident { $($err: ident),+ }) => (
		...
		impl fmt::Display for $name {
			fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
				match self {
					$(
						$name::$err(e) => write!(f, concat!("A ", stringify!($err), ": {}"), e),
					)+
				}
			}
		}
...
err_gen!(BuildError { AutoCommandBufferBuilderContextError, OomError });
...
err_gen!(FillBufferError { AutoCommandBufferBuilderContextError, CheckFillBufferError });
...
err_gen!(UpdateBufferError { AutoCommandBufferBuilderContextError, CheckUpdateBufferError });

...
use std::{error, fmt, marker::PhantomData, mem, ptr, sync::Arc, vec::IntoIter as VecIntoIter};
...
impl UnsafeCommandPool {
	...
	pub fn new(
		device: Arc<Device>, queue_family: QueueFamily, transient: bool, reset_cb: bool
		...
		let flags = {
			...
			let flag2 = if reset_cb { vk::COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT } else { 0 };
			flag1 | flag2
		};
	...
	pub unsafe fn reset(&self, release_resources: bool) -> Result<(), OomError> {
		let flags =
			if release_resources { vk::COMMAND_POOL_RESET_RELEASE_RESOURCES_BIT } else { 0 };
	...
	pub fn alloc_command_buffers(
		&self, secondary: bool, count: usize
...
unsafe impl VulkanObject for UnsafeCommandPool {
	...
	fn internal_object(&self) -> vk::CommandPool { self.pool }
}
...
unsafe impl VulkanObject for UnsafeCommandPoolAlloc {
	...
	fn internal_object(&self) -> vk::CommandBuffer { self.0 }
}
...
impl Iterator for UnsafeCommandPoolAllocIter {
	...
	fn size_hint(&self) -> (usize, Option<usize>) { self.list.size_hint() }
}
...
#[cfg(test)]
mod tests {
	use crate::command_buffer::pool::{CommandPoolTrimError, UnsafeCommandPool};

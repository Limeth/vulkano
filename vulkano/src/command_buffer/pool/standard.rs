...
unsafe impl CommandPoolBuilderAlloc for StandardCommandPoolBuilder {
	...
	fn inner(&self) -> &UnsafeCommandPoolAlloc { self.inner.inner() }

	fn into_alloc(self) -> Self::Alloc { self.inner }

	fn queue_family(&self) -> QueueFamily { self.inner.queue_family() }
}
...
unsafe impl CommandPoolAlloc for StandardCommandPoolAlloc {
	fn inner(&self) -> &UnsafeCommandPoolAlloc { &*self.cmd }

	fn queue_family(&self) -> QueueFamily {
		let queue_family_id = self.pool.pool.lock().unwrap().queue_family().id();

		self.device.physical_device().queue_family_by_id(queue_family_id).unwrap()
	}
}
...
mod tests {
	use crate::{
		command_buffer::pool::{CommandPool, CommandPoolBuilderAlloc, StandardCommandPool},
		device::Device,
		VulkanObject
	};
	use std::sync::Arc;

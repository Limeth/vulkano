...
impl UnsafeQueryPool {
	...
	pub fn new(
		device: Arc<Device>, ty: QueryType, num_slots: u32
		...
		Ok(UnsafeQueryPool { pool, device, num_slots })
	}
	...
	pub fn queries_range(&self, first_index: u32, count: u32) -> Option<UnsafeQueriesRange> {
		...
		if first_index + count < self.num_slots() {
			Some(UnsafeQueriesRange { pool: self, first: first_index, count })
		} else {
			None
		}
	}
...
unsafe impl VulkanObject for UnsafeQueryPool {
	...
	fn internal_object(&self) -> vk::QueryPool { self.pool }
}
...
impl<'a> UnsafeQuery<'a> {
	pub fn pool(&self) -> &'a UnsafeQueryPool { &self.pool }
...
impl<'a> UnsafeQueriesRange<'a> {
	...
	pub fn first_index(&self) -> u32 { self.first }
...
impl OcclusionQueriesPool {
	...
	/// Returns the number of slots of that query pool.
	pub fn num_slots(&self) -> u32 { self.inner.num_slots() }
}
